...
export default function ActiveChatHeader({
    ...
    // Chat lock hook and state
    const { isLocked, lockChat, unlockChat, hasFolderPasscode, setFolderPasscode, verifyFolderPasscode } = useChatLock();
    ...
    const handleLockToggleClick = async () => {
        setMenuOpen(false);
        const locked = isLocked(conversationId);
        ...
        if (locked) {
            ...
            const hasPass = await hasFolderPasscode();
            if (hasPass) {
                await lockChat(conversationId);
            } else {
                setPasscodeMode('set');
                setPasscodeError('');
                setIsPasscodeModalOpen(true);
    ...
    const handlePasscodeSubmit = async (passcode) => {
        ...
        if (passcodeMode === 'set') {
            ...
            const verified = await verifyFolderPasscode(passcode);
            if (verified) {
                     ...
                     {menuOpen && (
                         <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-xl z-50 py-1">
                             <button
                                 ...
                                 className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors cursor-pointer flex items-center gap-3"
                             >
                                 <span className="material-symbols-outlined text-[18px]">
                                     {isLocked(conversationId) ? 'lock_open' : 'lock'}
                                 </span>
                                 {isLocked(conversationId) ? 'Unlock Conversation' : 'Lock Conversation'}
                             </button>
                             ...
                             <button
                                 ...
                                 className="w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-gray-50 transition-colors cursor-pointer flex items-center gap-3 border-t border-gray-100"
                             >
